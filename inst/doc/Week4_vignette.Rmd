...
Note: If we had more than two predictors, it would be a good idea to calculate variance inflation factors. The package 'car' has a function 'vif' that takes as argument a fitted model. The package 'car' is not automatically installed with 'LandGenCourse' due to compatibility issues. Feel free to uncomment and run the code below to install 'car' and calculate variance inflation factors. Here, both predictors have VIF = 1.007, which indicates no collinearity.
...
- Creates an index 'a' that identifies two potential outliers by their row names,
...
- Exports the last plot as a 'png' (Portable Network Graphics) file, specifying canvas size and resolution.
