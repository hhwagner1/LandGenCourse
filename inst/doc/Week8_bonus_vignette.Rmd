...
Copy an R Notebook file (`Week3_vignette.Rmd`) to the downloads folder. This is just so that we have an example of an `.Rmd` file to extract the code from.
...
Extract the R code from the R Notebook and save it in a script file `Week3_vignette.R` in the downloads folder. You may adapt the infile and outfile to extract the code from any R Notebook saved in your project. 
...
With `source(outfile)`, we can source and run the entire file. Here, we extract only the first 94 lines of the R script. 
...
- How much time did the call to `PopGenReport::null.all` use? 
- How much of this time was used by the function itself, and how much by functions being called by it?
