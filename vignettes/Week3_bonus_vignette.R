...
## ------------------------------------------------------------------------
Matrix <- as.matrix(Pairwise.fst)
diag(Matrix) <- NA
Fst.per.site <- apply(Matrix, 1, mean, na.rm=TRUE)
names(Fst.per.site) <- levels(Frogs.genind$pop)
round(Fst.per.site, 3)

## ---- fig.show='hold'----------------------------------------------------
par(mar=c(4, 4.5,1,1))
plot(Frogs.sites$n, Fst.per.site, xlim=c(5,25),  ylim=c(0, 0.25),
       xlab = "Sample size", ylab = "Average pair-wise Fst")
abline(lm(Fst.per.site ~ summary(Frogs.genind)$n.by.pop), col = "red")
text(x=summary(Frogs.genind)$n.by.pop, y=Fst.per.site,
     labels=names(Fst.per.site), pos=4)

plot(Frogs.sites$AREA_m2, Fst.per.site, xlim=c(5,25),  ylim=c(0, 0.25),
       xlab = "Sample size", ylab = "Average pair-wise Fst")
abline(lm(Fst.per.site ~ summary(Frogs.genind)$n.by.pop), col = "red")
text(x=summary(Frogs.genind)$n.by.pop, y=Fst.per.site,
     labels=names(Fst.per.site), pos=4)
