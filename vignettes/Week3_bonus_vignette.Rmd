...
See also: Foll M and Gaggiotti O. 2006. Identifying the environmental factors that determine the genetic structure of populations. Genetics 174: 875-891. http://www.genetics.org/content/174/2/875

The degree of differentiation of each local population (deme) from all others can be expressed in an Fst value for each site. This has been implemented in the software GESTE (for Windows): 
http://www-leca.ujf-grenoble.fr/moyens-techniques/logiciels/article/population-genomics-software?lang=fr

Balkenhol (citation?) has used a simpler approach that averages the pairwise Fst values for each population. We already have calculated pairwise Fst values. It is an object of class 'dist'. The function 'as.matrix' converts from the distance vector format to the full distance matrix. We set the diagonal to missing values 'NA'. Then we use 'apply' to take the mean across each row = site, using option 'na.rm' to ignore missing values. 
...
Let's plot it against sample size (left) and pond area (right).
...
This is a counter-intuitive result, as we expect small populations to have higher drift and thus stronger differentiation. However, sample size is not the same as (effective) population size, and we did not take into account isolation either.
