...
dist_amova_fix <- function(x) {
  if (is(x, "locus")) x <- data.frame(LOCUS = x)

  if (!is(x, "data.frame")) {
    stop(paste(
      "The function dist_amova() requires a locus vector or a data frame of locus vectors. You passed a '",
      class(x), "' object.", sep = ""
    ))
  }
  ...
  if (any(p$Ploidy != round(p$Ploidy))) {
    message("PLOIDY WARNING: Assuming 2Gener Approximation.")
    data <- FALSE
    ...
    for (locus_name in loci) {
      locus <- x[[locus_name]]
      freqs <- gstudio::frequencies(locus)
      y <- gstudio::to_mv(locus)
      p_locus <- gstudio::ploidy(locus)

      for (i in seq_len(N)[p_locus == 2]) {
    ...
    data <- gstudio::to_mv(x, drop.allele = FALSE, leave.as.na = TRUE)
  }
  ...
  for (i in seq_len(N)) {
    ...
    for (j in seq_len(i)) {
      if (i != j) {
        yj <- data[j, ]
        ret[i, j] <- ret[j, i] <- sum(2 * t(xi - yj) %*% (xi - yj))
      }
    }
  }
...
assignInNamespace("dist_amova", dist_amova_fix, ns = "gstudio")
