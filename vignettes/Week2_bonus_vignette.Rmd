...
Go ahead and play with the interactive map!
...
- We write the figure into an object, `Map1`, then plot the `Map1`. This will help e.g. with exporting the map.
...
Go ahead and toggle between the base maps in the map below! Different base maps are suitable for different situations (data type, symbol type, size of study area) and purposes.
...
Let's beef it up a bit. We can do so by adding layers to `Map2`:
