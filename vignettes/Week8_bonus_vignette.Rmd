...
We use the function 'purl' from the 'knitr' package to extract the R code from a Notebook (here, we'll use the Week 1 worked example as this runs quickly):
...
**Question**: Which lines of code in the Week 1 worked example took the longest to run?
...
Here's an annotated example of a bash file that contains instructions for submitting a job to a single node (a single machine with multiple/many cores). The file 'BashExample.sh' should have been copied into the downloads folder inside your project folder.   

```{r BashExample}
file.show(file.path(here::here(), "downloads", "BashExample.sh"))
```
...
### d) Store your session info and package versions
